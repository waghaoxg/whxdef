//#include "..\detectFeaturePoints\detectFeaturePoints.h"
...
int main(){
	...
	std::string imgPath = "E:\\pic\\Íõò«ÐÇ´ð±çÕÕÆ¬\\IMG_5049.JPG";

	Mat img_color = imread(imgPath, 1);
	Mat img_gray= imread(imgPath, 0);
...
int blockSize = 3;
double  harrisK = 0.04;
	detectFeaturePoints( img_gray, corners, maxCorners,  qualityLevel, minDistance,
                              Mat(), blockSize, false, harrisK );
	//Mat disp_img = img_color.clone();
	//for (size_t i = 0; i < corners.size(); i++)
	//{
	//	cv::circle(disp_img, corners[i], 4, cv::Scalar(255,0,0), 2,8);
	//}
